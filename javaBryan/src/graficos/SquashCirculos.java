...
public class SquashCirculos {
	...
	private static boolean controlarRebote() {
		...
		if (Math.abs((bolaEnJuego.getCentro().getY()) - bolaEnJuego.getRadio() - raq.getExtIzq().getY()) < 4) {// Pelota
																												// a la
																												// altura
																												// de la
																												// raqueta
			if (bolaEnJuego.getCentro().getX() >= raq.getExtIzq().getX()
					&& bolaEnJuego.getCentro().getX() <= raq.getExtIzq().getX() + raq.getLargo()) {
				// toca raqueta
				StdAudio.playInBackground("soniquete.wav");
				bolaEnJuego.getCentro().setMovY(-bolaEnJuego.getCentro().getMovY());
			} else
