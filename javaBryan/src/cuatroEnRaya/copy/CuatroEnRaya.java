...
public class CuatroEnRaya {
	...
	public static void main(String[] args) {
		...
		/**
		 * Reglas esenciales : Punto final = Punto inicial ± Tamaño del segmento
		 * 
		 * Si el segmento va hacia la derecha (X aumenta) → se suma . Si el segmento va
		 * hacia la izquierda (X disminuye) → se resta . Si el segmento va hacia arriba
		 * (Y aumenta) → se suma . Si el segmento va hacia abajo (Y disminuye) → se
		 * resta . Punto medio = (PuntoA + PuntoB) / 2 Para una diagonal , también
		 * puedes usar la misma fórmula:puntoMedio = new Punto((p1.getX() + p2.getX()) /
		 * 2, (p1.getY() + p2.getY()) / 2); Se usa para calcular el centro de un
		 * segmento , ya sea horizontal, vertical o diagonal.
		 * 
		 */
		...
		// Se resta el alto total(tablero.length*TAM_CASILLA) del tablero porque la
		// coordinada Y decrece al ir hacia abajo
		// tablero[0].length * TAM_CASILLA ES EL ANCHO DEL TABLERO =Nº DE CLUMNAS POR SU
		// ANCHO
	...
	private static Integer detectarColumnaRaton() {
		...
		if (mouseX <= recTablero.getSupIzq().getX() || mouseX >= recTablero.getInfDer().getX()
			...
			return null; // Ratón fuera del tablero MANEJO LIMITES DEL RATON
		...
		double distanciaBordeIzquierdo = (mouseX - recTablero.getSupIzq().getX()) / TAM_CASILLA; // Produce un número entre 0.00 y 6.99
			...
			//CALCULO EL ANCHO PARA EL RECTANGULO RESALTAR
